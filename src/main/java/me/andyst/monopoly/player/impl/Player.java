...
public class Player implements IPlayer {
     ...
     * @return This player's previous position.
     */
    ...
    public int move(int roll) {
        ...
        board.getLotAt(prevPosition).decrementPlayerCount();
        ...
        board.getLotAt(position).incrementPlayerCount();
        return prevPosition;
    }
     ...
     * @return This player's previous position.
     */
    public int tick(Dice dice) {
        if (!isInJail || jailTurns == GameSettings.JAIL_TURNS) {
            free();
            return turn(dice.roll());
        }
        jailTurns++;
        return -1;
    }
